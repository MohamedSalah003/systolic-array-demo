# 3x3 Systolic Array in Verilog

This example implements the same 3x3 matrix-multiplication demo used in `systolic_vs_cpu.html`.

## Files

- `pe.v`: one processing element that forwards `a` and `b` and accumulates `a*b`
- `systolic_array_3x3.v`: nine PEs connected as a 3x3 array
- `tb_systolic_array_3x3.v`: testbench with the same numeric example used in the slides
...
One processing element in [pe.v](/Users/mohamedsalah/code/IC/ASIC_Diploma/me/verilog/session5/labs/systolic_array_3x3/pe.v#L1) looks like this internally:
...
The testbench multiplies the same matrices shown in the presentation:
...
To align the data correctly, the rows of `A` and the columns of `B` are skewed in time exactly like the presentation logic:
...
iverilog -g2012 -o sim pe.v systolic_array_3x3.v tb_systolic_array_3x3.v
vvp sim
```
